...
export function parseFiles(files: any): SourcifyOutput {
  const metadata = JSON.parse(
    files.find((file: File) => file.name === "metadata.json").content
  );
  const constructorArgs = files.find(
    (file: File) => file.name === "constructor-args.txt"
  );
  const sourcesArray = files.filter(
    (file: File) =>
      file.name !== "metadata.json" && file.name !== "constructor-args.txt"
  );
