...
export default class Verify extends Command {
  ...
  static flags = {
    ...
    network: flags.enum({
      ...
      options: [
        ...
        ...NETWORKS.map((n) => n.chainId.toString()),
      ],
      default: "mainnet",
    }),
    args: flags.string({
  ...
  async run() {
    ...
    const network: Network = NETWORKS.find(
      (n) => n.name === flags.network || n.chainId.toString() === flags.network
    )!;
    ...
    const { target, version, metadata, sources, constructorArgs } =
    ...
    this.log(target);
    this.log(version);
    this.log(args.contract);
    this.log(constructorArgs);
    writeFileSync(`${args.contract}.json`, JSON.stringify(standardJson));
    process.exit(0);
    ...
    const result = await etherscan.waitFor(
      network.etherscanApiURL,
      flags.apikey,
      guid
    );
